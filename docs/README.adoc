 ...
 * Raspberry Pi docs - bundle exec giblish -l warn -r data/resources/ -s giblish -f html -m --server-search-path /search/gibsearch.cgi -g 'develop' -d -c -j '/(accessories|computers|microcontrollers)/.*/' ../raspberry_doc/documentation/asciidoc ../gendocs/docs/blah
...
To illustrate the use of doc id. You can refer to document one as <<:docid:D-001>>. This will display a clickable link with the doc id (D-001 in this case).
