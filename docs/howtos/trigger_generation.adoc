...
To describe how to use `giblish` as a tool for creating a static web site with documents that are automatically generated each time a contributor push changes to a git repo.
...
Possible drawbacks::
 * The hook and publish scripts provided with giblish runs synchronously at each push from a _Doc Writer_ to the _Master Repo_. The time it takes to generate the HTML docs from the adoc source will thus be added to each push to the _Master Repo_.
...
script setup::
 . Find out where the template scripts for the `post-receive` hook by running `gem which giblish`. Strip of the `lib/giblish.rb` from the end of the returned path and `cd` to the resulting folder. There should be a sub folder named `scripts` under which you can find the template scripts somewhere.
+
