...
This text covers two deployment scenarios; one using git hooks only, and one using a combination of git hooks and Jenkins. They have been tested on Linux servers (Ubuntu). Most of the tools and scripts should work on a Windows server as well but might need some tweaking and is, again, not tested.

=== Using git hooks only
