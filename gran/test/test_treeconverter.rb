...
module Gran
  class TreeConverterTest < GranTestBase
    ...
    def test_build_from_fs
      src_tree = PathTree.build_from_fs(Pathname.new(__dir__) / "testdata", prune: false)
      assert(src_tree.count > 0)
      assert(src_tree.dir?(Pathname.new(__dir__) / "testdata"))
      logger.info("\n" + src_tree.to_s)
    end
